...
public class ZeroAndOnePackageClass {
    public static void main(String[] args) {
        ...
        int[][] enumResultFront = new int[v.length][packages + 1];
        // 递推公式f(i,j)=Math.max(f(i+1,j),f(i+1,j-v[i])+w[i])
        ...
        //使用递归算法求解
        int resultEnd = zeroAndOnePackageClass.Load_Bag_end(v, w, 0, packages);
        for (int i = 0; i < v.length; i++) {
            for (int j = 0; j <= packages; j++) {
                enumResultFront[i][j] = -1;
            }
        }
        Set<String> set = new HashSet<>();
        //使用带备忘录的递归算法求解
        zeroAndOnePackageClass.getBestValueEnd(v, w, 0, packages, enumResultFront);
        //递推公式f(i,j)=Math.max(f(i-1,j),f(i-1,j-v[i])+w[i])
        ...
        //使用带备忘录的递归算法求解
        zeroAndOnePackageClass.getBestValue(v, w, v.length, packages, enumResultFront);
    ...
    public int[] getEndByEnd(int[][] results, int[] v) {
        ...
        //若result[i][j]==result[i+1][j]时，说明第i个物品未装入背包中，设置数组下标位置为0,代表未装入；
        //当result[i][j]！=result[i+1][j]时，说明第i个物品装入背包中，设置数组下标位置为1，代表装入；当装入第i物品以后，需要寻找i物品的后继节点即第i+个节点，
        //第i+1个节点的背包容量为j-w[i];
     ...
     * @param packages 背包容量
     * @param result   结果
     *                 递推公式：f(i,j)=Math.max(f(i-1,j),f(i-1,j-v[i])+w[i])
     * @return
     */
     ...
     * @param i 物品数量
     * @param w 背包容量
     * @return 递推公式： f(i,j)=Math.max(f(i-1,j),f(i-1,j-v[i])+w[i])
     */
     ...
     * @return 递推公式： f(i,j)=Math.max(f(i+1,j),f(i+1,j-v[i])+w[i])
     */
    public int Load_Bag_end(int[] v, int[] w, int i, int capacity) {
        if (i == v.length || capacity == 0) {
            return 0;//如果考虑装用容量为capacity的背包装0个物品或者用0容量的背包装i个物品，所得价值为0
        }
        if (capacity < v[i]) {
            return Load_Bag_end(v, w, i + 1, capacity);//如果容积为capacity得背包装不下w[i]的物体，则其情况和用
            //capacity的背包装i-1的情况相同
        } else {
            return Math.max(Load_Bag_end(v, w, i + 1, capacity), (Load_Bag_end(v, w, i + 1, capacity - v[i]) + w[i]));//分两种情况，考虑装第i个物品，或不装
        }
    }
     ...
     * @return
     */
    public int getBestValueEnd(int[] v, int[] w, int index, int packages, int[][] result) {
        if (index == v.length || packages <= 0) {
            return 0;
        }
        if (result[index][packages] != -1) {
            return result[index][packages];
        }
        //当前背包不装该物品时的最大价值。
        int res = 0;
        if (packages < v[index]) {
            res = getBestValueEnd(v, w, index + 1, packages, result);
        } else {
            res = Math.max(getBestValueEnd(v, w, index + 1, packages, result), getBestValueEnd(v, w, index + 1, packages - v[index], result) + w[index]);
        }

        result[index][packages] = res;
        return res;
    }
}
